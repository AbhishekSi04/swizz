...
const Banner = ({
  ...
  useEffect(() => {
    if (user?.role === 'instructor') {
      (async () => {
        try {
          const res = await httpClient.get('/courses/mine');
          setStats({ courses: res.data?.length || 0, students: 0, rating: 4.5 });
        } catch (e) {
          setStats({ courses: 0, students: 0, rating: 0 });
        }
      })();
    }
  }, [user]);
              ...
              <Row className="d-flex justify-content-between">
                <Col xs={'auto'} className="mt-4 mt-md-0">
                  <div className="avatar avatar-xxl mt-n3">
                    <img className="avatar-img rounded-circle border border-white border-3 shadow" src={user?.avatarUrl || avatar1} alt="avatar" />
                  </div>
                </Col>
                <Col className="d-md-flex justify-content-between align-items-center mt-4">
                  <div>
                    <h1 className="my-1 fs-4">
                      {user?.name || 'User'} <BsPatchCheckFill className="text-info small" />
                    </h1>
