...
const CreateCourseForm = () => {
  ...
  const [courseData, setCourseData] = useState({ title: '', description: '', price: 0, category: 'general', lessons: [], published: false });
  const updateCourse = (patch) => setCourseData(prev => ({ ...prev, ...patch }));
  const addLesson = (title) => setCourseData(prev => ({ ...prev, lessons: [...prev.lessons, { title, content: '', durationMinutes: 0 }] }));
  useEffect(() => {
          ...
          <div id="stepper" ref={stepperRef} className="bs-stepper stepper-outline">
            ...
            <CardBody>
              <div className="bs-stepper-content">
                <Step1 stepperInstance={stepperInstance} courseData={courseData} updateCourse={updateCourse} />
                <Step2 stepperInstance={stepperInstance} />
                <Step3 stepperInstance={stepperInstance} courseData={courseData} addLesson={addLesson} />
                <Step4 stepperInstance={stepperInstance} courseData={courseData} />
              </div>
            </CardBody>
          </div>
        </Card>
